...
const PostView = ({ idStatus }) => {
  ...
  const PostViewTag = ({ data }) => {
    ...
    return tagarr.map(item => (
      <span
        key={item}
        onClick={() => window.location.replace(`/noticelist?tag=${item}`)}
      >
        {item}
      </span>
    ));
    ...
    return data.map(item => (
      <Fragment key={item.fileName}>
        <div
          ...
          onClick={() => onDownload(item.changedFile, item.fileName)}
        >
          <svg
  ...
  const addCommit = async () => {
    ...
    if (idStatus === undefined) {
      ...
      const data = JSON.stringify({
        data: commit.current.value,
        writter: idStatus,
        postNum: nowPageNumbers,
        postNumber: parseInt(params) + 1,
        postType: "게시판",
      });
      ...
      <div className="postViewSection">
        <header className="postViewName">
          <div className="postsettingIcon">
            ...
            <svg
              xmlns="http://www.w3.org/2000/svg"
              onClick={() => postDelete(postContentArr.numbers)}
              width="20"
              height="20"
              fill="currentColor"
