...
const NewPost = ({ idStatus }) => {
  ...
  const ShowFileList = ({ data, delitem }) => {
    // 첨부파일 정보
    return data.map(item => (
      <Fragment key={item[0].name}>
        ...
        <svg
          ...
          height="16"
          fill="currentColor"
          className="bi bi-x-square postattachmentdelete"
          viewBox="0 0 16 16"
        >
    ...
    if (showFileList.length >= 5) {
      ...
      showFileList.every(item => {
        if (item[0].name === e.target.files[0].name) {
          ...
          e.target.files[0].name.split(".")[1] === undefined ||
          e.target.files[0].name.split(".")[1] === "exe" ||
          e.target.files[0].name.split(".")[1] === "bat"
        ) {
  ...
  const addLabel = e => {
    if (e.key === "Enter") {
      if (labelData.length >= 5) {
        ...
        setLabelMode(false);
      } else if (labelInputData.current.value === "") {
        // 라벨이 빈값일 때
        setLabelMode(false);
      } else {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const accessToken = await localStorage.getItem("Authorization");
  ...
  const checkModifiedOrNewpost = async () => {
    ...
    if (urlResult[1] === "modified") {
      ...
      if (idStatus !== serverResult.data.postData.writer) {
        // url를 통해 불법적 접근은 제어
        alert("비정상적인 접근입니다.");
        ...
        if (getAttachMent !== null) {
          const attachResult = getAttachMent.map(item => {
            return [
              {
                name: item.fileName,
              },
        ...
        setPostContent({
          ...postContent,
          title: serverResult.data.postData.title,
        });
        setPostContent({
          ...postContent,
          content: serverResult.data.postData.content,
        });
        setPostOption({
          ...postOption,
          blockComm: serverResult.data.postData.blockComm,
          privates: serverResult.data.postData.privates,
        });
  ...
  const newPost = async () => {
    ...
    if (urlResult[1] === "newpost") {
      ...
      if (newPostResult.data.result === -1 || attachResult === -1) {
        alert(
          "게시물을 등록하는 도중에 오류가 발생했습니다. 다시 시도해 주세요."
        );
        window.history.back();
  ...
  return (
    <section className="newPostArea">
      <div className="formwrapper">
        ...
        >
          {postContent.content.length} / 1000
        </span>
      </div>
      <div className="postController">
        <Link to={"/noticelist"} className="linkButtondesign">
          <button className="postviewbutton_cancel">Cancel</button>
        </Link>
        <Link to={"/newpost"} className="linkButtondesign" onClick={newPost}>
          <button className="postviewbutton">Update</button>
        </Link>
        ...
        <div className="postSettings">
          ...
          <label className="postsettingcontent">
            <input
              type="checkbox"
              name="privates"
              checked={postOption.privates || false}
          ...
          <label className="postsettingcontent">
            <input
              type="checkbox"
              name="blockComm"
              checked={postOption.blockComm || false}
              onChange={onChangeOption}
            ></input>
        ...
        <div className="centerLine" />
        <span className="publicinSection">
          Title &#40; {postContent.title.length} / 30 &#41;
        </span>
        <input
        ...
        <div className="labelArea">
          ...
          {labelAddMode ? (
            ...
            <span className="labelsContent" onClick={setLabelMode}>
              {" "}
              {/* 라벨 추가 */}
              <svg
                xmlns="http://www.w3.org/2000/svg"
