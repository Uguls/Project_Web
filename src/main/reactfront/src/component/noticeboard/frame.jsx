...
const NoticeFrame = () => {
  ...
  const query = qs.parse(window.location.search, {
    // ?tag=데이터 로 찾음 query.tag
    ignoreQueryPrefix: true,
  });

  const noticeInfo = window.location.pathname.split("/")[1];
  if (
    (noticeInfo === undefined || noticeInfo === "") &&
    urlInfo !== "profile" &&
    urlInfo !== "newpost"
  ) {
    // 비정상적인 경로 확인
  ...
  const setDropers = () => {
    dropBoxs ? setDropBox(false) : setDropBox(true);
  };
      ...
      <div className="noticeFrame">
        <header className="noticeFrameHeader">
          <div className="myStory">
            <span className="noticeFrameName">
              <Link to="/noticeFrame" style={{ textDecoration: "none" }}>
                건양대학교
              </Link>
            </span>
          </div>
          {getCookieStat !== undefined ? (
            <div className="MobiledropBox" onClick={setMobileMenuBar}>
              <svg
                ...
                height="16"
                fill="gray"
                className="bi bi-chevron-double-down mobileDropBox"
                viewBox="0 0 16 16"
              >
                <path
                  fillRule="evenodd"
            ...
            <div className="logincommitsettingMobile">
              <Link to="/login" id="notDecor">
                Login
              </Link>
            </div>
          )}
          ...
          {mobileMenu ? (
            <div className="mobileDropBoxContent">
              <span className="menuMobile" onClick={setMobileMenuBar}>
                <Link to="/postlist" id="notDecor">
                  게시판
                </Link>
              </span>

              <ul>
                <Link to="/newpost" id="textDesign">
                  <li className="menuMobile">New Post</li>
                </Link>
                ...
                <Link
                  to="/login"
                  id="textDesign"
                  onClick={() => deleteAllToken()}
                >
                  <li className="menuMobile">Logout</li>
                </Link>
              </ul>
            </div>
          ) : null}
          ...
          <div id="buttonArea">
            {notified ? (
              <Notificate
                dropDownSet={setNotified}
                notifiedMode={userOption.notified}
                userId={getCookieStat}
              />
            ) : null}
            <span>
              <Link to="/noticelist" id="notDecor">
                게시판
              </Link>
            </span>
          </div>
            ...
            <span id="loginUser">
              {getCookieStat !== undefined ? (
                  ...
                  <div className="noticeLogin" onClick={setDropers}>
                    <div className="profileimage">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        fill="currentColor"
                        className="bi bi-person-circle peopleicon"
                        viewBox="0 0 16 16"
                      >
                    ...
                    <span style={{ borderRight: "none" }}>
                      ...
                      <svg
                        ...
                        height="16"
                        fill="currentColor"
                        className="bi bi-caret-down-fill profilepointer"
                        viewBox="0 0 16 16"
                      >
                ...
                <div className="logincommitsetting">
                  <Link to="/login" className="noticeLogin">
                    Login
                  </Link>
                  <Link to="/Register" className="noticeRegister">
                    Register
                  </Link>
                </div>
              )}
        ...
        <Routes>
          ...
          <Route
            path="/newpost"
            element={<NewPost idStatus={getCookieStat} />}
          ></Route>
          <Route
            path="/modified/:id"
            element={<NewPost idStatus={getCookieStat} />}
          ...
          <Route
            path="/viewpost"
            element={<PostView idStatus={getCookieStat} />}
          ></Route>
          <Route
            path="/profile"
            element={
              <Profile idStatus={getCookieStat} rerenders={getInitData} />
            }
          ></Route>
        </Routes>
      </div>
